...
to its digit/letter shape (#/X) and only the known report captions left
...
SSN_DASHED_RE = re.compile(r"\d{3}-\d{2}-\d{4}")
...
def is_sap_audit_page(lines, header_x):
    """True if this page really is a SAP audit report page. Without this
    gate any PDF with a dashed 9-digit number on it -- a W2, a transcript
    -- would yield junk 'student' rows if it were sitting in the source
    folder."""
    if header_x:
        return True
    text = " ".join(line_text(line) for line in lines).lower()
    return "satisfactory academic progress" in text


def find_header_columns(lines):
    """Locate the "ID SSN Name" caption row and return the x of each
    caption, used as a last-resort splitter when the ID and an undashed
    SSN are printed with no space between them. Returns None if the
    caption row isn't on the page."""
    for line in lines:
...
def split_id_ssn_name(line, header_x):
    ...
    lowered = text.lower()
    looks_like_student = bool(HONORIFIC_RE.match(text)) or "academic program" in lowered
    if not (header_x and looks_like_student):
        return None
...
def process_pdf(path: Path):
    ...
    for page_num, page in enumerate(doc, start=1):
        ...
        if not is_sap_audit_page(lines, header_x):
...
def debug_page(path: Path, page_num: int):
    ...
    print(f"recognised as a SAP audit page: "
          f"{'yes' if is_sap_audit_page(lines, header_x) else 'NO -- this page would be skipped'}")
