...
def resolve_pdf_paths(inputs):
    """Accept individual PDF paths and/or folder paths (expanded to the .pdf files inside)."""
    resolved = []
    for item in inputs:
        p = Path(item)
        if p.is_dir():
...
def main():
    ...
    parser.add_argument("pdfs", nargs="+", help="Path(s) to input PDF file(s) and/or folder(s) containing PDFs")
